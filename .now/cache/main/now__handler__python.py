...
if 'handler' in __now_variables or 'Handler' in __now_variables:
    base = main.handler if ('handler' in __now_variables) else  main.Handler
    ...
    if (
        not inspect.iscoroutinefunction(main.app) and
        not inspect.iscoroutinefunction(main.app.__call__)
    ):
        ...
        def now_handler(event, context):
            ...
            response = Response.from_app(main.app, environ)
        ...
        def now_handler(event, context):
            ...
            response = asgi_cycle(main.app, body)
            return response
